...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.pertandingan.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.pertandingans.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('hasil') ? 'is-invalid' : '' }}" name="hasil" id="hasil">
                    ...
                    @foreach(App\Models\Pertandingan::HASIL_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('hasil', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('hasil'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('medali') ? 'is-invalid' : '' }}" name="medali" id="medali">
                    ...
                    @foreach(App\Models\Pertandingan::MEDALI_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('medali', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
