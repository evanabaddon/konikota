    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.events.update", [$event->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('tgl_mulai') ? 'is-invalid' : '' }}" type="text" name="tgl_mulai" id="tgl_mulai" value="{{ old('tgl_mulai', $event->tgl_mulai) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('tgl_selesai') ? 'is-invalid' : '' }}" type="text" name="tgl_selesai" id="tgl_selesai" value="{{ old('tgl_selesai', $event->tgl_selesai) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('venues') ? 'is-invalid' : '' }}" name="venues[]" id="venues" multiple>
                    @foreach($venues as $id => $venue)
                        <option value="{{ $id }}" {{ (in_array($id, old('venues', [])) || $event->venues->contains($id)) ? 'selected' : '' }}>{{ $venue }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('cabors') ? 'is-invalid' : '' }}" name="cabors[]" id="cabors" multiple>
                    @foreach($cabors as $id => $cabor)
                        <option value="{{ $id }}" {{ (in_array($id, old('cabors', [])) || $event->cabors->contains($id)) ? 'selected' : '' }}>{{ $cabor }}</option>
                    @endforeach
                </select>
